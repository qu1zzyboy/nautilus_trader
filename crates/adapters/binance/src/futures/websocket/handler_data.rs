...
use super::{
    messages::{
        ...
        BinanceFuturesAlgoUpdateMsg, BinanceFuturesBookTickerMsg, BinanceFuturesContinuousKlineMsg,
        BinanceFuturesDepthUpdateMsg, BinanceFuturesExecWsMessage, BinanceFuturesKlineMsg,
                    ...
                    serde_json::from_value::<BinanceFuturesContinuousKlineMsg>(json.clone())
                {
                    ...
                    match parse_continuous_kline(&msg, instrument) {
                        Ok(Some(bar)) => {
                            ...
                            return Some(NautilusWsMessage::Data(
                                NautilusDataWsMessage::Data(vec![Data::Bar(bar)]),
                            ));
                        }
                        Ok(None) => {
