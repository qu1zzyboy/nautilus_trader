// -------------------------------------------------------------------------------------------------
//  Copyright (C) 2015-2026 Nautech Systems Pty Ltd. All rights reserved.
//  https://nautechsystems.io
//
//  Licensed under the GNU Lesser General Public License Version 3.0 (the "License");
//  You may not use this file except in compliance with the License.
//  You may obtain a copy of the License at https://www.gnu.org/licenses/lgpl-3.0.en.html
//
//  Unless required by applicable law or agreed to in writing, software
//  distributed under the License is distributed on an "AS IS" BASIS,
//  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
//  See the License for the specific language governing permissions and
//  limitations under the License.
// -------------------------------------------------------------------------------------------------

//! Factory functions for creating MEXC clients and components.

use std::{cell::RefCell, rc::Rc};

use nautilus_common::{
    cache::Cache,
    clients::{DataClient, ExecutionClient},
    clock::Clock,
};
use nautilus_live::ExecutionClientCore;
use nautilus_model::{
    enums::{AccountType, OmsType},
    identifiers::ClientId,
};
use nautilus_system::factories::{ClientConfig, DataClientFactory, ExecutionClientFactory};

use crate::{
    common::consts::{MEXC, MEXC_VENUE},
    config::{MexcDataClientConfig, MexcExecClientConfig},
    data::MexcDataClient,
    execution::client::MexcExecutionClient,
};

/// Factory for creating MEXC data clients.
#[derive(Debug)]
pub struct MexcDataClientFactory;

impl MexcDataClientFactory {
    /// Creates a new [`MexcDataClientFactory`] instance.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
}

impl Default for MexcDataClientFactory {
    fn default() -> Self {
        Self::new()
    }
}

impl DataClientFactory for MexcDataClientFactory {
    fn create(
        &self,
        name: &str,
        config: &dyn ClientConfig,
        _cache: Rc<RefCell<Cache>>,
        _clock: Rc<RefCell<dyn Clock>>,
    ) -> anyhow::Result<Box<dyn DataClient>> {
        let mexc_config = config
            .as_any()
            .downcast_ref::<MexcDataClientConfig>()
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "Invalid config type for MexcDataClientFactory. Expected MexcDataClientConfig, was {config:?}",
                )
            })?
            .clone();

        let client_id = ClientId::from(name);

        let client = MexcDataClient::new(client_id, mexc_config)?;
        Ok(Box::new(client))
    }

    fn name(&self) -> &'static str {
        MEXC
    }

    fn config_type(&self) -> &'static str {
        stringify!(MexcDataClientConfig)
    }
}

/// Factory for creating MEXC execution clients.
#[derive(Debug)]
pub struct MexcExecutionClientFactory;

impl MexcExecutionClientFactory {
    /// Creates a new [`MexcExecutionClientFactory`] instance.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
}

impl Default for MexcExecutionClientFactory {
    fn default() -> Self {
        Self::new()
    }
}

impl ExecutionClientFactory for MexcExecutionClientFactory {
    fn create(
        &self,
        name: &str,
        config: &dyn ClientConfig,
        cache: Rc<RefCell<Cache>>,
        clock: Rc<RefCell<dyn Clock>>,
    ) -> anyhow::Result<Box<dyn ExecutionClient>> {
        let mexc_config = config
            .as_any()
            .downcast_ref::<MexcExecClientConfig>()
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "Invalid config type for MexcExecutionClientFactory. Expected MexcExecClientConfig, was {config:?}",
                )
            })?
            .clone();

        // MEXC Spot uses cash account type and hedging OMS
        let account_type = AccountType::Cash;
        let oms_type = OmsType::Hedging;

        let core = ExecutionClientCore::new(
            mexc_config.trader_id,
            ClientId::from(name),
            *MEXC_VENUE,
            oms_type,
            mexc_config.account_id,
            account_type,
            None, // base_currency
            clock,
            cache,
        );

        let client = MexcExecutionClient::new(core, mexc_config)?;
        Ok(Box::new(client))
    }

    fn name(&self) -> &'static str {
        MEXC
    }

    fn config_type(&self) -> &'static str {
        stringify!(MexcExecClientConfig)
    }
}

#[cfg(test)]
mod tests {
    use nautilus_system::factories::DataClientFactory;
    use rstest::rstest;

    use super::*;

    #[rstest]
    fn test_mexc_data_client_factory_creation() {
        let factory = MexcDataClientFactory::new();
        assert_eq!(factory.name(), "MEXC");
        assert_eq!(factory.config_type(), "MexcDataClientConfig");
    }

    #[rstest]
    fn test_mexc_data_client_factory_default() {
        let factory = MexcDataClientFactory;
        assert_eq!(factory.name(), "MEXC");
    }

    #[rstest]
    fn test_mexc_execution_client_factory_creation() {
        let factory = MexcExecutionClientFactory::new();
        assert_eq!(factory.name(), "MEXC");
        assert_eq!(factory.config_type(), "MexcExecClientConfig");
    }
}

